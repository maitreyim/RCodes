...
generateTradeSignal <- function(tick){
  ...
  if(dfList$full)    
  {
    ...
    for(i in 2:nrow (resFrame))
    {
      ...
      resFrame[i,11]=(resFrame[i,9]-resFrame[i-1,9])/resFrame[i,9]
      #Calculate Lag 2 of Return of FAS
      resFrame[i,12]=(resFrame[i,10]-resFrame[i-1,10])/resFrame[i,10]
